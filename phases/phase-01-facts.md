...
- **Extract `EnsureSameCurrency` as a private guard method.** `Add` and `Subtract` share the same currency-match invariant. Extracted now rather than waiting for rule-of-three because the invariant is structural to `Money` (every binary operation combining two `Money` values must agree on currency) — not a speculative third-instance prediction, but documenting the type's algebra. The guard takes an operation-name parameter so error messages remain operation-specific (`"Cannot add USD and GBP"`, `"Cannot subtract USD and GBP"`). Reversible if a future binary operation needs a different shape.
- **Added `Negate` as a unary arithmetic operation.** Identified via test smell — the algebraic-equivalence test (`Subtract_IsEquivalentTo_AddingNegative`) had to manually construct `new Money(-amount, currency)`, signaling a missing abstraction. `Negate` is the standard counterpart to `Add`/`Subtract` for value-type arithmetic, improves test expressiveness (`b.Negate()` vs `new Money(-b.Amount, b.Currency)`), and will earn its keep in Phase 2+ (e.g. expressing a withdrawal as "add the negation of this Money").
- *(add others as we go)*
